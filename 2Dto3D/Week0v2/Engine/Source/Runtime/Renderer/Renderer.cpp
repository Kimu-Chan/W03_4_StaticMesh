...
{
    // 占쏙옙占쏙옙 占쌩삼옙占쏙옙 占쏙옙占심쇽옙占쏙옙 占쏙옙占쏙옙
    if (Graphics == nullptr)
        assert(0);
...
{
    D3D11_BUFFER_DESC indexbufferdesc = {};						// buffer占쏙옙 占쏙옙占쏙옙, 占쎈도 占쏙옙占쏙옙 占쏙옙占쏙옙
    indexbufferdesc.Usage = D3D11_USAGE_IMMUTABLE;			        // immutable: gpu占쏙옙 占싻깍옙 占쏙옙占쏙옙占쏙옙占쏙옙 占쏙옙占쏙옙占쏙옙 占쏙옙 占쌍댐옙.
    indexbufferdesc.BindFlags = D3D11_BIND_INDEX_BUFFER;	        // index buffer占쏙옙 占쏙옙占쏙옙構渼占�.
    indexbufferdesc.ByteWidth = byteWidth;	// buffer 크占쏙옙 占쏙옙占쏙옙
...
void FRenderer::UpdateLightBuffer()
{
    ...
    {
        constants->lightDirX = 1.0f; // 占쏙옙: 占쏙옙占쏙옙 占쏙옙占쏙옙占쏙옙 占싣뤄옙占쏙옙 占쏙옙占쏙옙占쏙옙占쏙옙 占쏙옙占�
        constants->lightDirY = 1.0f; // 占쏙옙: 占쏙옙占쏙옙 占쏙옙占쏙옙占쏙옙 占싣뤄옙占쏙옙 占쏙옙占쏙옙占쏙옙占쏙옙 占쏙옙占�
        constants->lightDirZ = 1.0f; // 占쏙옙: 占쏙옙占쏙옙 占쏙옙占쏙옙占쏙옙 占싣뤄옙占쏙옙 占쏙옙占쏙옙占쏙옙占쏙옙 占쏙옙占�
...
void FRenderer::UpdateConstant(FMatrix _MVP, float _Flag)
{
    if (ConstantBuffer)
    {
        D3D11_MAPPED_SUBRESOURCE constantbufferMSR;// GPU占쏙옙 占쌨몌옙 占쌍쇽옙 占쏙옙占쏙옙
        ...
        FConstants* constants = (FConstants*)constantbufferMSR.pData; //GPU 占쌨몌옙 占쏙옙占쏙옙 占쏙옙占쏙옙
        {
            constants->MVP = _MVP;
            constants->Flag = _Flag;
        }
        Graphics->DeviceContext->Unmap(ConstantBuffer, 0); // GPU占쏙옙 占쌕쏙옙 占쏙옙諛∽옙占쏙옙構占� 占쏙옙占쏙옙占�
    }
}
...
void FRenderer::UpdateNormalConstantBuffer(FMatrix _Model)
{
    ...
    {
        D3D11_MAPPED_SUBRESOURCE constantbufferMSR; // GPU 占쏙옙 占쌨몌옙 占쌍쇽옙 占쏙옙占쏙옙
        ...
        FNormalConstants* constants = (FNormalConstants*)constantbufferMSR.pData; //GPU 占쌨몌옙 占쏙옙占쏙옙 占쏙옙占쏙옙
        {
            constants->ModelMatrixInverseTranspose = NormalMatrix;
        }
        Graphics->DeviceContext->Unmap(NormalConstantBuffer, 0); // GPU 占쏙옙 占쌕쏙옙 占쏙옙諛∽옙占쏙옙構占� 占쏙옙占쏙옙占�. 
...
void FRenderer::UpdateLitUnlitConstantBuffer(int isLit)
{
    if (LitUnlitBuffer) {
        ...
        FLitUnlitConstants* constants = (FLitUnlitConstants*)constantbufferMSR.pData; //GPU 占쌨몌옙 占쏙옙占쏙옙 占쏙옙占쏙옙
        {
...
void FRenderer::PrepareTextureShader()
{
    ...
    //占쌔쏙옙占식울옙 ConstantBuffer 占쌩곤옙占십울옙占쌀쇽옙占쏙옙
    if (ConstantBuffer)
    {
        Graphics->DeviceContext->VSSetConstantBuffers(0, 1, &ConstantBuffer);
    }
...
void FRenderer::RenderTextPrimitive(ID3D11Buffer* pVertexBuffer, UINT numVertices, ID3D11ShaderResourceView* _TextureSRV, ID3D11SamplerState* _SamplerState)
{
    ...
    // 占쌉뤄옙 占쏙옙占싱아울옙 占쏙옙 占썩본 占쏙옙占쏙옙
    Graphics->DeviceContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
    Graphics->DeviceContext->PSSetShaderResources(0, 1, &_TextureSRV);
    Graphics->DeviceContext->PSSetSamplers(0, 1, &_SamplerState);

    // 占쏙옙恝占� 호占쏙옙 (6占쏙옙占쏙옙 占싸듸옙占쏙옙 占쏙옙占�)
    Graphics->DeviceContext->Draw(numVertices, 0);
}
...
void FRenderer::UpdateSubUVConstant(float _indexU, float _indexV)
{
    if (SubUVConstantBuffer)
    {
        ...
        FSubUVConstant* constants = (FSubUVConstant*)constantbufferMSR.pData; //GPU 占쌨몌옙 占쏙옙占쏙옙 占쏙옙占쏙옙
        {
            constants->indexU = _indexU;
            constants->indexV = _indexV;
        }
        Graphics->DeviceContext->Unmap(SubUVConstantBuffer, 0); // GPU占쏙옙 占쌕쏙옙 占쏙옙諛∽옙占쏙옙構占� 占쏙옙占쏙옙占�
    }
}
...
{
    // 占쏙옙占싱댐옙占쏙옙 占쌉뤄옙 占쏙옙占싱아울옙 占쏙옙占쏙옙
    ...
    // 占쏙옙占� 占쏙옙占쏙옙 占쏙옙占싸듸옙: 
    // - MatrixBuffer占쏙옙 register(b0)占쏙옙, Vertex Shader占쏙옙 占쏙옙占싸듸옙
    // - GridConstantBuffer占쏙옙 register(b1)占쏙옙, Vertex占쏙옙 Pixel Shader占쏙옙 占쏙옙占싸듸옙 (占싫쇽옙 占쏙옙占싱댐옙占쏙옙 占십요에 占쏙옙占쏙옙)
...
ID3D11Buffer* FRenderer::CreateBoundingBoxBuffer(UINT numBoundingBoxes)
{
    ...
    bufferDesc.Usage = D3D11_USAGE_DYNAMIC; // 占쏙옙占쏙옙 占쏙옙占쏙옙占쏙옙트占쏙옙 占쏙옙占� DYNAMIC, 占쌓뤄옙占쏙옙 占쏙옙占쏙옙占쏙옙 DEFAULT
...
ID3D11ShaderResourceView* FRenderer::CreateBoundingBoxSRV(ID3D11Buffer* pBoundingBoxBuffer, UINT numBoundingBoxes)
{
    ...
    srvDesc.Format = DXGI_FORMAT_UNKNOWN; // 占쏙옙占쏙옙체 占쏙옙占쏙옙占쏙옙 占쏙옙占� UNKNOWN
...
void FRenderer::UpdateGridConstantBuffer(const FGridParameters& gridParams)
{
    ...
    if (SUCCEEDED(hr)) {
        ...
        //UE_LOG(LogLevel::Warning, "gridParams 占쏙옙占쏙옙 占쏙옙占쏙옙");
    }
